...
| Block rewriting | `EditMergedSpans` rebuilt a `BT`…`ET` block from only the edited span's glyphs, so every other run in that block — other lines of the same paragraph — was deleted. It also reconstructed position from the block's first `Tm`, which is absent when a block positions with `Td`, dropping the rewritten text to the block origin. Blocks are now rebuilt from every run they contain, each at its own recorded matrix, with the block prologue copied verbatim. `TestEditMergedSpans_PreservesOtherLinesInTheBlock`, `TestEditMergedSpans_PreservesPositionsOfUntouchedLines`, `TestEditMergedSpans_PreservesBlockPrologue` | §9.4.2 |
